...
**Professional timing model** — Every question has two independent clocks. Players get a **60-second answer window** to lock in YES or NO. The window then runs for **3–5 match minutes** measured against the real match clock — not wall time — anchored from the moment the player submits, so the result always reflects exactly what the market did after they committed. All questions settle when the window closes: the resolver checks whether the condition was met by that point, so "happens" questions (goal, corner, odds move) and "hold" questions follow the same rhythm. A wall-clock hard cap guarantees no question can ever hang on a stalled feed, and a 5-second resolution sweep announces results on time even when the feed goes quiet.

**Match lifecycle engine** — One central transition engine reacts the moment the match changes phase. Kickoff resets the game state and arms the first question for ~2 minutes after the whistle. Halftime settles all open predictions and pauses questions. The second-half restart re-arms everything instantly. Full time force-settles every open prediction, kills any question card still on screen across all clients, silences all commentary, and hands straight over to a next-match countdown that ticks every second inside the final minute and flips directly into the live feed at kickoff.

**Broadcast-accurate match clock** — The scoreboard behaves exactly like television. First-half stoppage displays **45+1' → 45+6'**, never a false jump to 46'. The feed's clock convention (per-half vs cumulative) is auto-detected and latched per match. Halftime flips instantly the moment the feed reports the clock stopped at 45'+, and the second half switches to 2H and starts at 46' regardless of whether the feed sends an explicit period signal. Halftime and full time are latched once declared — feeds that keep reporting `Period: 1, Running: true` through the break cannot drag the display back. The latch releases only on genuine second-half evidence, and a premature full-time call self-heals back to live the moment the clock or market moves again.

**The odds stream as a match-state oracle** — Kaching's most original use of TxLINE data: in-play markets trade continuously through stoppage time but **suspend at halftime and close at full time**, which makes the odds stream itself the most reliable break signal on fixtures whose clock feed is ambiguous. While the market is actively trading, no clock heuristic is allowed to declare the match over — the game is provably live. When the clock freezes *and* the market goes quiet, the break is confirmed within about two minutes. A slow pure-clock fallback remains as an unconditional escape hatch for fixtures with no odds flow. The data layer that powers the gameplay also referees the match state.
...
**Market-accurate probabilities** — The win-probability bar shows the true market consensus. Implied probabilities are extracted from live TxLINE prices and normalised to remove the bookmaker overround, matching professional trading screens. In demo mode, a synthetic probability engine starts near 50/50 and drifts realistically each tick, spiking 12–20% on goals so the bar tells the story of the match. If a live message cannot be parsed, the engine returns nothing rather than inventing a value — the bar only ever moves on real data.
...
**Score persistence with self-healing** — Scores are written to a SQLite database via sql.js after every prediction result, with an auto-save every 30 seconds as a safety net. On page refresh, the session is restored via a GET endpoint that never creates a new player. If the hosting platform ever resets the disk while a browser still holds its session, the scoring layer detects the missing player row and recreates it on the spot. Correct answers add points with a streak multiplier up to 3×. Wrong answers reset the streak to zero but never reduce the score — a player's earned points are permanent.
...
- The odds stream's suspend-at-break behaviour turned out to be a reliable match-state signal in its own right — a hidden strength of the data
...
- Clock semantics vary by fixture — `Clock.Period` and `Running` are sometimes reported unchanged through the halftime break, and numeric fields can arrive as strings. Kaching handles this with numeric coercion, frozen-clock detection, break latching, and by cross-referencing the odds stream's suspension state — but documented clock-state semantics would save every builder this work
